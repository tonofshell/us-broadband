...
ggplot(data = urban_bb, aes(x = factor(`Year`), y = `Total Charge`)) +
  ...
  scale_color_manual(values = color_pal(4, "cool")) +
  scale_fill_manual(values = color_pal(4, "cool")) +
...
ggplot(data = urban_bb, aes(x = factor(`Year`), y = `Download Bandwidth Mbps`)) +
  geom_violin(aes(color = ordered(Year), fill = ordered(Year))) +
  scale_color_manual(values = color_pal(4, "warm")) +
  scale_fill_manual(values = color_pal(4, "warm")) +
...
urban_bb[c(2,18)] %>% table() %>% as_tibble() %>% get_perc("Year", "n") %>%
...
ny_map = st_read(here("Datasets", "US_Shapefile", "tl_2018_us_county.shp")) %>% 
  subset(STATEFP %in% "36") %>% ggplot() + geom_sf(color = "grey90", fill = "white")
...
ny_county_bb$INTPTLON = -as.numeric(substr(ny_county_bb$INTPTLON, start = 3, stop = 11))
ny_county_bb$INTPTLAT = as.numeric(substr(ny_county_bb$INTPTLAT, start = 2, stop = 11))
```
...
ny_map + geom_sf(data = ny_county_bb, 
                 aes(fill = `% Hse Units Cable` / 100,
                     ...
                     alpha = pop_dens),
                 size = 1) + 
 scale_fill_gradientn(colors = color_pal(8, type = "continuous"), 
                        labels = percent_format(accuracy = 1)) +
  scale_color_gradientn(colors = color_pal(8, type = "continuous"), 
                        labels = percent_format(accuracy = 1)) +
...
#us_map = st_read(here("Datasets", "US_Shapefile", "tl_2018_us_county.shp")) %>% ggplot() + geom_sf(color = "grey90", fill = "white")

states_sf %>% 
  ggplot() + 
  geom_sf(color = "grey90", fill = "white") +
  #coord_sf(crs = st_crs(102003)) +
  geom_point(data = acs_17_mod, aes(x = INTPTLON,
                     y = INTPTLAT,
                     color = (1- no_inet),
                     size = pop_dens)) + 
