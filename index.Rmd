...
ggplot(data = urban_bb, aes(x = `Total Charge`, y = `Download Bandwidth Mbps`)) + 
  ...
  geom_point(color = color_pal(1), alpha = 0.5) + 
...
urban_bb[c(2,18)] %>% table() %>% as_tibble() %>% get_perc("Year", "n") %>%
  ...
  labs(title = "Data Caps are Not Disappearing",
       ...
       fill = "Has Data Cap",
       y = "Percentage of Respondents") +
  scale_y_continuous(labels = percent_format(), breaks = c(0, 0.2, 0.4, 0.6)) +
  theme_master() 
```
...
ny_map + geom_point(data = ny_county_bb, aes(x = INTPTLON,
                     y = INTPTLAT,
                     color = `% Hse Units Cable`,
                     size = pop_dens)) + 
  scale_color_gradientn(colors = color_pal(8, type = "continuous"), 
                        breaks = c(50, 65, 85, 100), limits = c(50,100), 
                        labels = c("< 50", "65", "85", "100")) +
   geom_point(data = subset(ny_county_bb, `% Hse Units Cable` < 50), aes(x = INTPTLON,
                     y = INTPTLAT,
                     size = pop_dens),
              color = color_pal(8, type = "continuous")[1]) + 
