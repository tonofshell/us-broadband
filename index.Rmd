...
ny_map + geom_point(data = ny_county_bb, aes(x = INTPTLON,
                     y = INTPTLAT,
                     color = `% Hse Units Cable` / 100,
                     size = pop_dens)) + 
  scale_color_gradientn(colors = color_pal(8, type = "continuous"), 
                        labels = percent_format(accuracy = 1)) +
  scale_size(trans = "log10", range = c(0.5, 8)) +
  labs(title = "Less Populous Counties have Less Cable Internet Coverage",
       subtitle = "Rural counties with less population density proportionately have less households with cable internet",
       color = "Percent with \nCable Internet",
...
ny_map + geom_point(data = ny_county_bb, aes(x = INTPTLON,
  ...
  scale_color_gradientn(colors = color_pal(8, type = "continuous"), 
                        labels = percent_format(accuracy = 1)) +
  scale_size(trans = "log10", range = c(0.5, 8)) +
  labs(title = "Less Populous Counties have Less Fiber Internet Coverage",
       subtitle = "Rural counties with less population density proportionately have less households with fiber internet",
       color = "Percent with \nFiber Internet",
       size = "Population Density \n(ppl/sq mi)",
       caption = "New York State Broadband Availability By Municipality") +
    theme_map() +
...
states_sf %>% 
  ...
  scale_size(range = c(0.5, 15), breaks = c(50, 500, 5000, 50000)) +
  scale_alpha(range = c(1.0, 0.05)) +
  labs(title = "Rural Areas have Less Access to Broadband Internet",
