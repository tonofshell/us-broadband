...
ggplot(data = people_net, aes(x = percent_no_internet, y = percent_below_poverty)) + geom_point() + geom_smooth(method='lm',formula=y~x) + labs(title = "Relationship Between Poverty and Internet Access", subtitle = "By US County", x = "Percent with no Internet", y = "Percent Below Poverty Line")
ggsave("Charts/internet vs poverty.pdf")
...
ggplot(data = urban_bb, aes(x = `Total Charge`, y = `Download Bandwidth Mbps`)) + geom_point(color = "light grey") + 
  geom_smooth(method='lm', formula=y~x, color = "dark grey") + 
  geom_point(data = subset(urban_bb, State == "New York"), color = "blue") + 
  labs(title = "Relationship Between Download Speed and Price", subtitle = "By Cable Provider per State", x = "Total Monthly Internet Charge (US)", y = "Download Speed (mbps)")
ggsave("Charts/download vs price.pdf")
...
ny_map + geom_polygon(data = ny_county_bb, aes(fill = ny_county_bb$pop_dens), color = "white") + geom_polygon(color = "black", fill = NA) + scale_fill_distiller(palette = "Spectral", direction = -1, trans = "log10") + theme_light() + ditch_the_axes 
ggsave("Charts/ny pop dens county.pdf")
...
ggsave("Charts/ny cable prov boxplot.pdf")
...
ggsave("Charts/ny fiber prov boxplot county.pdf")
